...
// 3D Scaled Simplex raw noise custom permutation.
//
// Returned value will be between loBound and hiBound.
float scaled_raw_noise_3d( const float loBound, const float hiBound, const float x, const float y, const float z , int *custom_perm) {
    return raw_noise_3d(x, y, z, custom_perm) * (hiBound - loBound) / 2 + (hiBound + loBound) / 2;
}
...
// 3D raw Simplex noise with custom permutations
float raw_noise_3d( const float x, const float y, const float z , int *custom_perm)
{
    ...
    int jj = j & 255;
    int kk = k & 255;
    int gi0 = custom_perm[ii+custom_perm[jj+custom_perm[kk]]] % 12;
    int gi1 = custom_perm[ii+i1+custom_perm[jj+j1+custom_perm[kk+k1]]] % 12;
    int gi2 = custom_perm[ii+i2+custom_perm[jj+j2+custom_perm[kk+k2]]] % 12;
    int gi3 = custom_perm[ii+1+custom_perm[jj+1+custom_perm[kk+1]]] % 12;
